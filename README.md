...
傳統的科學化力量訓練（如 Stronglift 5x5）成效卓著，但繁瑣的重量計算、失敗降重（Deload）判定與數據追蹤，往往為訓練者帶來沉重的認知負荷。

本專案旨在打造一個輕量級的終端機 AI 健力教練。透過 Separation of Concerns 架構，系統將自然語言的意圖解析交由大語言模型（LLM）處理，而核心的 5x5 訓練守則與數值計算則由 TypeScript 工具接管。降低生成式 AI 的數學幻覺，也透過動態上下文注入與嚴密的防護網，提供訓練者流暢、安全且具備長期記憶的對話式指導體驗。

## ✨ Core Features

* **Conversational Logging & Guardrails (自然語言紀錄與資料護欄)**
訓練者可使用口語回報訓練狀況。系統內建嚴謹的資料 Guardrails 機制，當偵測到缺漏動作或極端不合理的數據時，會主動攔截並反問訓練者，徹底避免資料庫遭到不可逆的污染。
* **Deterministic Tool Calling (絕對精準的工具調用)**
捨棄 LLM 不可靠的運算能力，系統透過 Function Calling 呼叫 TypeScript 實作的工具（如 `addWeight`, `deload`, `recordFail`）。確保每一次的加減重計算 100% 符合 Stronglift 5x5 規則。
* **State Persistence & Snapshots (狀態持久化與進度快照)**
實作極度輕量的本地檔案 I/O 機制。每次訓練結算後，系統會於背景自動覆寫 `Current_State.json`，並將當日所有動作的重量進度作為 Snapshot 自動寫入 `Workout_History.json`，達成跨會話的長期追蹤。
...
整合開源的 `free-exercise-db.json`，捨棄龐大的外部向量資料庫，實作零延遲的記憶體內檢索。內建同義詞字典，精準對映口語發問與靜態資料庫間的差異，並具備優雅降級的容錯處理。
...
請先確認環境已安裝 Node.js (v18+)，接著複製專案並安裝相依套件：
